    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-4">
        <div className="text-center max-w-md mx-auto mb-3">
          ...
          <h3 className="text-sm font-bold font-sans text-navy-950 uppercase tracking-wide">
            Global Database Lookup
          </h3>
          <p className="text-[11px] text-gray-500 font-sans mt-1 leading-normal">
            Query across the unified registry: look up device unit barcodes, manufacturer serial stamps, or logistics dispatch batch codes.
          </p>
        </div>
        ...
        <SearchInput
          ...
          placeholder="Enter 3+ characters to search unit codes, serials, batch codes..."
        ...
        <div className="space-y-6">
          {searchQuery.isLoading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3 bg-white border rounded-lg shadow-sm">
              <Spinner size="md" />
              <p className="text-xs text-gray-500 font-sans">Querying ECI directories...</p>
            </div>
          ) : searchQuery.isError ? (
            <div className="p-5 text-center bg-white border border-gray-200 rounded-lg text-red-500 text-xs flex flex-col items-center justify-center gap-2 shadow-sm">
              ...
              <p className="text-gray-500 text-[11px] max-w-xs mt-1">
                Failed to perform global search queries. Verify if search API services are fully operational.
              </p>
            </div>
          ) : totalResults === 0 ? (
            <div className="bg-white border border-gray-200 rounded-lg p-10 text-center text-gray-400 select-none shadow-sm">
              <p className="text-xs font-bold font-sans">No Records Found</p>
              <p className="text-[10px] text-gray-500 font-sans mt-1">
                No matching registries found for query &ldquo;<span className="font-mono text-gray-700 font-bold">{query}</span>&rdquo;. Check spelling or prefix identifiers.
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              {results.evmUnits.length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-4">
                  <h4 className="text-xs font-bold font-sans text-navy-950 uppercase tracking-wide pb-2 border-b border-gray-100 flex items-center gap-2">
                    ...
                    Matching EVM Device Units ({results.evmUnits.length})
                  </h4>
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-gray-500 font-sans mt-1">
                            Serial: <span className="font-mono font-bold text-gray-600">{unit.serialNumber}</span> &bull;{' '}
                            Custody: {unit.districtName ? `${unit.districtName}, ` : ''}{unit.stateName}
                          </p>
                        </div>
              ...
              {results.dispatchBatches.length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-4">
                  <h4 className="text-xs font-bold font-sans text-navy-950 uppercase tracking-wide pb-2 border-b border-gray-100 flex items-center gap-2">
                    ...
                    Matching Dispatch Batches ({results.dispatchBatches.length})
                  </h4>
                    ...
                    {results.dispatchBatches.map((batch) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-gray-500 font-sans mt-1">
                            Flow: {batch.fromDistrictName || 'State Pool'} &rarr; {batch.toDistrictName || 'State Pool'} &bull;{' '}
                            Sent: {batch.dispatchDate ? format(new Date(batch.dispatchDate), 'dd MMM yyyy') : '—'}
                          </p>
                        </div>
                        <ArrowRight size={14} className="text-gray-300 shrink-0" />
                      </div>
